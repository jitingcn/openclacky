...
const Sessions = (() => {
  ...
  function _renderAssistantMessage(content, reasoningContent = "") {
    ...
    if (extracted.reasoning && extracted.reasoning.trim()) {
      html += _buildThinkingBlock(_markedParse(extracted.reasoning));
    }
  ...
  function _restoreAssistantBubbleState(el, state) {
    ...
    if (thinkingBlock) thinkingBlock.open = !!state.thinkingOpen;
  ...
  function _applyAssistantContentToShell(el, content, reasoningContent = "") {
    ...
    if (contentBody) {
      contentBody.innerHTML = extracted.content && extracted.content.trim()
