...
const Sessions = (() => {
  ...
  function _markedParse(text) {
    if (!text) return "";
    const protectedMath = (typeof MathRenderer !== "undefined" && MathRenderer.protectMarkdown)
    ...
    if (typeof marked !== "undefined") {
      ...
      const html = marked.parse(protectedMath.text, { breaks: true, gfm: true, renderer });
      return (typeof MathRenderer !== "undefined" && MathRenderer.restoreProtectedHtml)
        ? MathRenderer.restoreProtectedHtml(html, protectedMath.formulas)
        : html;
    }
  ...
  function _applyAssistantContentToShell(el, content, reasoningContent = "", { typesetMath = true } = {}) {
    ...
    if (thinkingBlock && thinkingBody) {
      ...
      if (hasReasoning) {
        thinkingBody.innerHTML = _markedParse(extracted.reasoning);
        if (userToggled) {
      ...
      _renderMathInElement(thinkingBody, typesetMath);
    }

    if (contentBody) {
      contentBody.innerHTML = _assistantHasVisibleContent(extracted)
        ? _renderMarkdown(extracted.content)
        : "";
      _renderMathInElement(contentBody, typesetMath);
    }
  }
  ...
  return {
    ...
    appendAssistantDelta({ contentDelta = null, reasoningDelta = null, content = null, reasoningContent = null, replace = false } = {}) {
      ...
      // Avoid MathJax re-typesetting the whole live bubble on every delta.
      // We keep raw TeX visible during streaming and typeset once on finalize.
