...
const Sessions = (() => {
  ...
  function _renderMarkdown(rawText) {
    ...
    while (true) {
      ...
      const closeIdx = rest.indexOf(closeTag, openIdx + openTag.length);
      ...
      const thinkContent = rest.slice(openIdx + openTag.length, closeIdx);
  ...
  function _protectMarkdownMath(text) {
    return (typeof MathRenderer !== "undefined" && MathRenderer.protectMarkdown)
      ? MathRenderer.protectMarkdown(text)
      : { text, formulas: [] };
  }

  function _restoreProtectedMarkdown(html, formulas) {
    return (typeof MathRenderer !== "undefined" && MathRenderer.restoreProtectedHtml)
      ? MathRenderer.restoreProtectedHtml(html, formulas)
      : html;
  }

  function _renderProtectedMarkdown(protectedMarkdown) {
    const text = protectedMarkdown?.text || "";
    const formulas = protectedMarkdown?.formulas || [];
    if (!text) return "";
    ...
    if (typeof marked !== "undefined") {
      ...
      return _restoreProtectedMarkdown(html, formulas);
    }

    return _restoreProtectedMarkdown(
      escapeHtml(text).replace(/\n/g, "<br>"),
      formulas
    );
  }

  function _buildProtectedMarkdownSegment(text, formulas) {
    const protectedText = text || "";
    return {
      text: protectedText,
      formulas: Array.isArray(formulas)
        ? formulas.filter((formula) => protectedText.includes(formula.token))
        : []
    };
  }

  function _canCommitTrailingStreamingToken(protectedText, tokens) {
    if (!protectedText || !tokens.length) return true;

    const lastToken = tokens[tokens.length - 1];
    if (!lastToken) return true;
    if (lastToken.type === "space") return true;

    return /\n\s*\n\s*$/.test(protectedText);
  }

  function _splitStreamingMarkdown(text) {
    const protectedMarkdown = _protectMarkdownMath(text);
    const protectedText = protectedMarkdown.text || "";
    if (!protectedText) {
      return {
        committed: { text: "", formulas: [] },
        active: { text: "", formulas: [] }
      };
    }
    ...
    if (typeof marked === "undefined" || typeof marked.lexer !== "function") {
      return {
        committed: { text: "", formulas: [] },
        active: protectedMarkdown
      };
    }

    let tokens;
    try {
      tokens = marked.lexer(protectedText, { breaks: true, gfm: true });
    } catch (_err) {
      return {
        committed: { text: "", formulas: [] },
        active: protectedMarkdown
    ...
    let committedCount = Math.max(0, tokens.length - 1);
    if (_canCommitTrailingStreamingToken(protectedText, tokens)) {
      committedCount = tokens.length;
    }
    ...
    const committedText = tokens
      ...
      .map((token) => token.raw || "")
      .join("");
    const activeText = protectedText.slice(committedText.length);

    return {
      committed: _buildProtectedMarkdownSegment(committedText, protectedMarkdown.formulas),
      active: _buildProtectedMarkdownSegment(activeText, protectedMarkdown.formulas)
    };
  }

  function _ensureStreamingMarkdownShell(root) {
    if (!root) return null;

    let committed = root.querySelector(".clacky-stream-committed");
    let active = root.querySelector(".clacky-stream-active");
    if (committed && active) return { committed, active };

    root.innerHTML =
      `<div class="clacky-stream-committed"></div>` +
      `<div class="clacky-stream-active"></div>`;
    root.__clackyCommittedProtectedText = "";
    root.__clackyStreamRawText = "";

    committed = root.querySelector(".clacky-stream-committed");
    active = root.querySelector(".clacky-stream-active");
    return { committed, active };
  }

  function _resetStreamingMarkdownShell(root) {
    if (!root) return;
    root.__clackyCommittedProtectedText = "";
    root.__clackyStreamRawText = "";
    root.innerHTML = "";
  ...
  function _appendStreamingMarkdownHtml(root, html, { typesetMath = true } = {}) {
    if (!root || !html) return;
    ...
    while (temp.firstChild) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        _renderMathInElement(node, typesetMath);
      }
    }
  }

  function _renderStreamingMarkdown(root, rawText, { typesetTail = false } = {}) {
    if (!root) return;

    const source = rawText || "";
    if (!source) {
      _resetStreamingMarkdownShell(root);
      return;
    }

    const shell = _ensureStreamingMarkdownShell(root);
    if (!shell) return;

    const previousRawText = root.__clackyStreamRawText || "";
    const split = _splitStreamingMarkdown(source);
    const nextCommittedText = split.committed.text || "";

    if (previousRawText && !source.startsWith(previousRawText)) {
      shell.committed.innerHTML = "";
      root.__clackyCommittedProtectedText = "";
    }

    const committedBaseText = root.__clackyCommittedProtectedText || "";
    if (!nextCommittedText.startsWith(committedBaseText)) {
      shell.committed.innerHTML = "";
      root.__clackyCommittedProtectedText = "";
    }

    const currentCommittedText = root.__clackyCommittedProtectedText || "";
    if (nextCommittedText.length > currentCommittedText.length) {
      const deltaText = nextCommittedText.slice(currentCommittedText.length);
      const deltaSegment = _buildProtectedMarkdownSegment(deltaText, split.committed.formulas);
      _appendStreamingMarkdownHtml(
        shell.committed,
        _renderProtectedMarkdown(deltaSegment),
        { typesetMath: true }
      );
      root.__clackyCommittedProtectedText = nextCommittedText;
    }

    shell.active.innerHTML = split.active.text ? _renderProtectedMarkdown(split.active) : "";
    _renderMathInElement(shell.active, typesetTail);
    root.__clackyStreamRawText = source;
  }

  function _markedParse(text) {
    if (!text) return "";
    return _renderProtectedMarkdown(_protectMarkdownMath(text));
  }
  ...
  function _buildThinkingBlock(renderedHtml, openByDefault = false) {
    return `<details class="thinking-block"${openByDefault ? " open" : ""}>` +
  ...
  function _assistantHasVisibleContent(extracted) {
    return !!(extracted && extracted.content && extracted.content.trim());
  }

  function _shouldAutoOpenThinkingBlock(extracted) {
    const hasReasoning = !!(extracted && extracted.reasoning && extracted.reasoning.trim());
    return hasReasoning && !_assistantHasVisibleContent(extracted);
  }

  function _setThinkingBlockOpenState(thinkingBlock, open) {
    if (!thinkingBlock || thinkingBlock.open === open) return;

    thinkingBlock.dataset.ignoreNextToggle = "true";
    thinkingBlock.open = open;
  }

  function _bindThinkingBlockState(el) {
    if (!el) return;

    const thinkingBlock = el.querySelector(".thinking-block");
    if (!thinkingBlock || thinkingBlock.dataset.toggleBound === "true") return;

    thinkingBlock.dataset.toggleBound = "true";
    thinkingBlock.dataset.userToggled = thinkingBlock.dataset.userToggled || "false";
    thinkingBlock.dataset.ignoreNextToggle = thinkingBlock.dataset.ignoreNextToggle || "false";

    thinkingBlock.addEventListener("toggle", () => {
      if (thinkingBlock.dataset.ignoreNextToggle === "true") {
        thinkingBlock.dataset.ignoreNextToggle = "false";
        return;
      }
  ...
  function _extractLeadingThinkingBlock(rawText) {
    const text = rawText || "";
    ...
    for (const [openTag, closeTag] of candidates) {
      if (!trimmed.startsWith(openTag)) continue;
      const closeIdx = trimmed.indexOf(closeTag, openTag.length);
      if (closeIdx === -1) continue;

      return {
        reasoning: trimmed.slice(openTag.length, closeIdx),
        content: trimmed.slice(closeIdx + closeTag.length).replace(/^\s+/, "")
      };
    }
  ...
  function _renderAssistantMessage(content, reasoningContent = "") {
    const extracted = reasoningContent ? { reasoning: reasoningContent, content: _extractLeadingThinkingBlock(content).content } : _extractLeadingThinkingBlock(content);
    let html = "";

    if (extracted.reasoning && extracted.reasoning.trim()) {
      html += _buildThinkingBlock(_markedParse(extracted.reasoning), _shouldAutoOpenThinkingBlock(extracted));
    }
    if (_assistantHasVisibleContent(extracted)) {
      html += _renderMarkdown(extracted.content);
    }
    return html;
  }

  function _captureAssistantBubbleState(el) {
    if (!el) return null;

    const thinkingBlock = el.querySelector(".thinking-block");
    const thinkingBody = el.querySelector(".thinking-body");
    return {
      thinkingOpen: !!thinkingBlock?.open,
      thinkingUserToggled: thinkingBlock?.dataset.userToggled === "true",
      thinkingScrollTop: thinkingBody ? thinkingBody.scrollTop : null,
      thinkingScrollLeft: thinkingBody ? thinkingBody.scrollLeft : null,
    };
  }

  function _restoreAssistantBubbleState(el, state) {
    if (!el || !state) return;
    ...
    if (thinkingBlock) {
      thinkingBlock.dataset.userToggled = state.thinkingUserToggled ? "true" : "false";
      _setThinkingBlockOpenState(thinkingBlock, !!state.thinkingOpen);
    }
    if (thinkingBody && state.thinkingScrollTop !== null) {
      thinkingBody.scrollTop = state.thinkingScrollTop;
      thinkingBody.scrollLeft = state.thinkingScrollLeft || 0;
    }
  }

  function _renderMathInElement(el, typesetMath = true) {
    if (!el || typeof MathRenderer === "undefined") return;

    if (typesetMath) {
      MathRenderer.typesetElement(el);
    } else if (typeof MathRenderer.hydrateElement === "function") {
      MathRenderer.hydrateElement(el);
    }
  }

  function _renderAssistantBubble(el, content, reasoningContent = "") {
    if (!el) return;
    const preservedState = _captureAssistantBubbleState(el);
    el.dataset.raw = content || "";
    el.innerHTML = _renderAssistantMessage(content || "", reasoningContent || "");
    _bindThinkingBlockState(el);
    _appendCopyButton(el);
    _renderMathInElement(el, true);
    _restoreAssistantBubbleState(el, preservedState);
  }

  function _applyAssistantContentToShell(el, content, reasoningContent = "", { typesetMath = true } = {}) {
    if (!el) return;

    const extracted = reasoningContent
      ? { reasoning: reasoningContent, content: _extractLeadingThinkingBlock(content).content }
      : _extractLeadingThinkingBlock(content);

    const thinkingBlock = el.querySelector(".thinking-block");
    const thinkingBody = el.querySelector(".thinking-body");
    const contentBody = el.querySelector(".assistant-stream-content");

    if (thinkingBlock && thinkingBody) {
      const wasOpen = thinkingBlock.open;
      const userToggled = thinkingBlock.dataset.userToggled === "true";
      const prevScrollTop = thinkingBody.scrollTop;
      const prevScrollLeft = thinkingBody.scrollLeft;
      const hasReasoning = !!(extracted.reasoning && extracted.reasoning.trim());
      const shouldAutoOpen = _shouldAutoOpenThinkingBlock(extracted);

      thinkingBlock.style.display = hasReasoning ? "" : "none";
      if (hasReasoning) {
        _renderStreamingMarkdown(thinkingBody, extracted.reasoning, { typesetTail: typesetMath });
        if (userToggled) {
          _setThinkingBlockOpenState(thinkingBlock, wasOpen);
        } else {
          _setThinkingBlockOpenState(thinkingBlock, shouldAutoOpen);
        }
        thinkingBody.scrollTop = prevScrollTop;
        thinkingBody.scrollLeft = prevScrollLeft;
      } else {
        _resetStreamingMarkdownShell(thinkingBody);
      }
    }

    if (contentBody) {
      if (_assistantHasVisibleContent(extracted)) {
        _renderStreamingMarkdown(contentBody, extracted.content, { typesetTail: typesetMath });
      } else {
        _resetStreamingMarkdownShell(contentBody);
      }
    }
  }
  ...
  let _mouseScrollDragging = false; // true while mouse drag on the message pane may be moving the scrollbar
  ...
  function _initNewMessageBanner() {
    ...
    messages.addEventListener("mousedown", (e) => {
      if (e.button === 0) _mouseScrollDragging = true;
    });
    window.addEventListener("mouseup", () => {
      _mouseScrollDragging = false;
    });
    ...
    messages.addEventListener("scroll", () => {
      if (_mouseScrollDragging && !_isAtBottom(messages)) {
        _userScrolledUp = true;
      }
  ...
  function _renderHistoryEvent(ev, container, historyCtx) {
    ...
    switch (ev.type) {
      ...
      case "assistant_message": {
        ...
        el.innerHTML = _renderAssistantMessage(ev.content || "", ev.reasoning_content || "");
        _bindThinkingBlockState(el);
        _appendCopyButton(el);
        if (typeof MathRenderer !== "undefined") MathRenderer.typesetElement(el);
        container.appendChild(el);
        break;
      }
  ...
  return {
    ...
    renderAssistantMessage(content, reasoningContent = "") {
      return _renderAssistantMessage(content, reasoningContent);
    },
    // ── Init ──────────────────────────────────────────────────────────────
    init() {
    ...
    _liveAssistantStream: null,  // { el, rawContent, reasoningContent } while assistant is streaming
    ...
    _ensureLiveAssistantStream() {
      const current = Sessions._liveAssistantStream;
      if (current && current.el && current.el.isConnected) return current;
      ...
      el.innerHTML =
        ...
        `</details>` +
        `<div class="assistant-stream-content"></div>`;
      _bindThinkingBlockState(el);
      _appendCopyButton(el);
      messages.appendChild(el);

      Sessions._liveAssistantStream = {
        el,
        rawContent: "",
        reasoningContent: ""
      };
      return Sessions._liveAssistantStream;
    },

    appendAssistantDelta({ contentDelta = null, reasoningDelta = null, content = null, reasoningContent = null, replace = false } = {}) {
      const stream = Sessions._ensureLiveAssistantStream();
      if (!stream) return;

      if (replace) {
        stream.rawContent = content || "";
        stream.reasoningContent = reasoningContent || "";
      } else {
        stream.rawContent += contentDelta || "";
        stream.reasoningContent += reasoningDelta || "";
      }

      stream.el.dataset.raw = stream.rawContent || "";
      // Stream only the unstable tail. Stable markdown blocks are moved into the
      // committed prefix and typeset once, so later deltas do not rebuild them.
      _applyAssistantContentToShell(stream.el, stream.rawContent, stream.reasoningContent, { typesetMath: false });
      _scrollToBottomIfNeeded($("messages"));
    },

    finalizeAssistantMessage(content, reasoningContent = "") {
      const stream = Sessions._liveAssistantStream;
      if (stream && stream.el && stream.el.isConnected) {
        stream.rawContent = content || "";
        stream.reasoningContent = reasoningContent || "";
        stream.el.dataset.raw = stream.rawContent || "";
        _applyAssistantContentToShell(stream.el, stream.rawContent, stream.reasoningContent);
        Sessions._liveAssistantStream = null;
        _scrollToBottomIfNeeded($("messages"));
        return;
      }
      ...
      Sessions.appendMsg(
        "assistant",
        Sessions.renderAssistantMessage(content || "", reasoningContent || ""),
        { html: true, raw: content || "" }
      );
    },

    resetAssistantStream() {
      const stream = Sessions._liveAssistantStream;
      if (stream && stream.el) stream.el.remove();
      Sessions._liveAssistantStream = null;
    },
    ...
    appendMsg(type, html, { time, html: isHtml = false, raw = null } = {}) {
      ...
      if (type === "assistant") {
        if (isHtml) {
          el.dataset.raw = raw || html || "";
          el.innerHTML = html;
          _bindThinkingBlockState(el);
          _appendCopyButton(el);
          if (typeof MathRenderer !== "undefined") MathRenderer.typesetElement(el);
        } else {
          _renderAssistantBubble(el, raw || html || "", "");
        }
      } else {
        el.innerHTML = html;
      }
    ...
    showFeedbackRequest(question, context, options) {
      ...
      if (typeof MathRenderer !== "undefined") MathRenderer.typesetElement(card);
