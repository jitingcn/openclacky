...
const Profile = (() => {
  ...
  function _renderMarkdown(raw) {
    ...
    const protectedMath = (typeof MathRenderer !== "undefined" && MathRenderer.protectMarkdown)
      ? MathRenderer.protectMarkdown(raw)
      : { text: raw, formulas: [] };
    const escaped = _escapeHtml(protectedMath.text);
  ...
  function _renderIdentitySection(kind) {
    ...
    if (typeof MathRenderer !== "undefined") MathRenderer.typesetElement(wrap);
          ...
          fetch("/api/memories/" + encodeURIComponent(m.filename))
            ...
            .then(d => {
              ...
              body.innerHTML = _renderMarkdown(stripped)
                || `<div class="profile-empty">${_t("profile.emptyContent")}</div>`;
              if (typeof MathRenderer !== "undefined") MathRenderer.typesetElement(body);
