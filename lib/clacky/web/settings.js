...
const Settings = (() => {
  ...
  function _renderCard(container, model, index) {
    ...
    card.innerHTML = `
      ...
      <div class="model-fields">
        ...
        <label class="model-field">
          <span class="field-label">API Type</span>
          <div class="custom-select-wrapper" data-index="${index}">
            <div class="custom-select-trigger custom-select-trigger--small">
              <span class="custom-select-value ${!_esc(model.api_type) ? 'placeholder' : ''}">${_esc(model.api_type) || "Auto-detect"}</span>
              <svg class="custom-select-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </div>
            <div class="custom-select-dropdown api-type-dropdown">
              <div class="custom-select-option" data-value="">Auto-detect</div>
              <div class="custom-select-option" data-value="openai-completions">OpenAI Chat Completions</div>
              <div class="custom-select-option" data-value="openai-responses">OpenAI Responses API</div>
              <div class="custom-select-option" data-value="anthropic-messages">Anthropic Messages</div>
              <div class="custom-select-option" data-value="bedrock">AWS Bedrock</div>
            </div>
          </div>
        </label>
        <label class="model-field">
          ...
          <div class="custom-select-wrapper" data-index="${index}">
            <div class="custom-select-trigger custom-select-trigger--small">
              <span class="custom-select-value">${model.stream === true ? "Always" : model.stream === false ? "Never" : "Auto"}</span>
              <svg class="custom-select-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </div>
            <div class="custom-select-dropdown stream-dropdown">
              <div class="custom-select-option" data-value="null">Auto (try streaming first)</div>
              <div class="custom-select-option" data-value="true">Always stream</div>
              <div class="custom-select-option" data-value="false">Never stream</div>
            </div>
          </div>
        </label>
        <label class="model-field">
          ...
          <div class="custom-select-wrapper" data-index="${index}">
            <div class="custom-select-trigger custom-select-trigger--small">
              <span class="custom-select-value">${model.prompt_caching === true ? "Enabled" : model.prompt_caching === false ? "Disabled" : "Auto"}</span>
              <svg class="custom-select-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </div>
            <div class="custom-select-dropdown prompt-caching-dropdown">
              <div class="custom-select-option" data-value="null">Auto (detect by format)</div>
              <div class="custom-select-option" data-value="true">Enabled</div>
              <div class="custom-select-option" data-value="false">Disabled</div>
            </div>
          </div>
        </label>
        <label class="model-field">
          ...
          <div class="custom-select-wrapper" data-index="${index}">
            <div class="custom-select-trigger custom-select-trigger--small">
              <span class="custom-select-value">${model.thinking_enabled === true ? "Enabled" : model.thinking_enabled === false ? "Disabled" : "Auto"}</span>
              <svg class="custom-select-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </div>
            <div class="custom-select-dropdown thinking-dropdown">
              <div class="custom-select-option" data-value="null">Auto</div>
              <div class="custom-select-option" data-value="true">Enabled</div>
              <div class="custom-select-option" data-value="false">Disabled</div>
            </div>
          </div>
        ...
        <label class="model-field">
          ...
          <div class="custom-select-wrapper" data-index="${index}">
            <div class="custom-select-trigger custom-select-trigger--small">
              <span class="custom-select-value ${!_esc(model.reasoning_effort) ? 'placeholder' : ''}">${_esc(model.reasoning_effort) || "Auto"}</span>
              <svg class="custom-select-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
              </svg>
            </div>
            <div class="custom-select-dropdown reasoning-effort-dropdown">
              <div class="custom-select-option" data-value="">Auto</div>
              <div class="custom-select-option" data-value="none">None</div>
              <div class="custom-select-option" data-value="minimal">Minimal</div>
              <div class="custom-select-option" data-value="low">Low</div>
              <div class="custom-select-option" data-value="medium">Medium</div>
              <div class="custom-select-option" data-value="high">High</div>
              <div class="custom-select-option" data-value="max">Max</div>
              <div class="custom-select-option" data-value="xhigh">Xhigh</div>
            </div>
          </div>
        </label>

        <!-- Per-model compression overrides (collapsible) -->
        <div class="model-compression-overrides" data-index="${index}">
          <button type="button" class="compression-overrides-toggle" data-index="${index}">
            <svg class="compression-overrides-arrow" fill="none" stroke="currentColor" viewBox="0 0 24 24" width="14" height="14">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <span>${I18n.t("settings.models.compressionOverrides.title")}</span>
            <span class="compression-overrides-hint">${I18n.t("settings.models.compressionOverrides.hint")}</span>
          </button>
          <div class="compression-overrides-body" style="display:none">
            <label class="model-field model-field--compact">
              <span class="field-label">${I18n.t("settings.models.compressionOverrides.tokenThreshold")}</span>
              <div class="settings-compression-input-row">
                <input type="number" class="field-input settings-compression-input" data-override="token_threshold" data-index="${index}"
                  placeholder="${I18n.t("settings.models.compressionOverrides.useGlobal")}" min="10000" step="10000"
                  value="${model.compression_overrides?.token_threshold || ''}">
                <span class="settings-compression-unit">tokens</span>
              </div>
            </label>
            <label class="model-field model-field--compact">
              <span class="field-label">${I18n.t("settings.models.compressionOverrides.messageThreshold")}</span>
              <div class="settings-compression-input-row">
                <input type="number" class="field-input settings-compression-input" data-override="message_threshold" data-index="${index}"
                  placeholder="${I18n.t("settings.models.compressionOverrides.useGlobal")}" min="20" step="10"
                  value="${model.compression_overrides?.message_threshold || ''}">
                <span class="settings-compression-unit">messages</span>
              </div>
            </label>
            <label class="model-field model-field--compact">
              <span class="field-label">${I18n.t("settings.models.compressionOverrides.maxRecent")}</span>
              <div class="settings-compression-input-row">
                <input type="number" class="field-input settings-compression-input" data-override="max_recent_messages" data-index="${index}"
                  placeholder="${I18n.t("settings.models.compressionOverrides.useGlobal")}" min="5" max="100" step="5"
                  value="${model.compression_overrides?.max_recent_messages || ''}">
                <span class="settings-compression-unit">messages</span>
              </div>
            </label>
            <label class="model-field model-field--compact">
              <span class="field-label">${I18n.t("settings.models.compressionOverrides.targetTokens")}</span>
              <div class="settings-compression-input-row">
                <input type="number" class="field-input settings-compression-input" data-override="target_tokens" data-index="${index}"
                  placeholder="${I18n.t("settings.models.compressionOverrides.useGlobal")}" min="1000" step="1000"
                  value="${model.compression_overrides?.target_tokens || ''}">
                <span class="settings-compression-unit">tokens</span>
              </div>
            </label>
            <label class="model-field model-field--compact">
              <span class="field-label">${I18n.t("settings.models.compressionOverrides.idleThreshold")}</span>
              <div class="settings-compression-input-row">
                <input type="number" class="field-input settings-compression-input" data-override="idle_threshold" data-index="${index}"
                  placeholder="${I18n.t("settings.models.compressionOverrides.useGlobal")}" min="5000" step="5000"
                  value="${model.compression_overrides?.idle_threshold || ''}">
                <span class="settings-compression-unit">tokens</span>
              </div>
            </label>
            <label class="model-field model-field--compact">
              <span class="field-label">${I18n.t("settings.models.compressionOverrides.idleDelay")}</span>
              <div class="settings-compression-input-row">
                <input type="number" class="field-input settings-compression-input" data-override="idle_delay" data-index="${index}"
                  placeholder="${I18n.t("settings.models.compressionOverrides.useGlobal")}" min="30" step="30"
                  value="${model.compression_overrides?.idle_delay || ''}">
                <span class="settings-compression-unit">seconds</span>
              </div>
            </label>
          </div>
        </div>
  ...
  function _bindCardEvents(card, index) {
    // ── All custom-select dropdowns on this card (provider, api_type, stream) ──
    card.querySelectorAll(".custom-select-wrapper").forEach(wrapper => {
    ...
    if (isNewCard && initialPromoHint && (!initialValue || initialValue === "openclacky")) {
      ...
      options.forEach(option => {
        option.addEventListener("click", (e) => {
          ...
          // Provider-specific: auto-fill model & base_url
          if (dropdown === card.querySelector(".quick-setup-field .custom-select-dropdown")) {
            ...
            // Show promo hint for openclacky, hide for others; show on empty default state
            if (promoHint) {
    ...
    // Toggle compression overrides section
    const coToggle = card.querySelector(".compression-overrides-toggle");
    const coBody   = card.querySelector(".compression-overrides-body");
    const coArrow  = card.querySelector(".compression-overrides-arrow");
    if (coToggle && coBody) {
      coToggle.addEventListener("click", () => {
        const isOpen = coBody.style.display !== "none";
        coBody.style.display  = isOpen ? "none" : "";
        coArrow.style.transform = isOpen ? "" : "rotate(90deg)";
      });
    }
  ...
  function _readCard(index) {
    ...
    // Read api_type from dropdown
    const apiTypeWrapper = card.querySelector(".api-type-dropdown")?.closest(".custom-select-wrapper");
    let apiType = null;
    if (apiTypeWrapper) {
      const selected = apiTypeWrapper.querySelector(".custom-select-option.selected");
      apiType = selected ? selected.dataset.value : (_models[index]?.api_type || null);
    }

    // Read stream from dropdown
    const streamWrapper = card.querySelector(".stream-dropdown")?.closest(".custom-select-wrapper");
    let stream = null;
    if (streamWrapper) {
      const selected = streamWrapper.querySelector(".custom-select-option.selected");
      if (selected) {
        stream = selected.dataset.value === "null" ? null : (selected.dataset.value === "true" ? true : false);
      } else {
        stream = _models[index]?.stream ?? null;
      }
    }

    // Read prompt_caching from dropdown
    const pcWrapper = card.querySelector(".prompt-caching-dropdown")?.closest(".custom-select-wrapper");
    let prompt_caching = null;
    if (pcWrapper) {
      const selected = pcWrapper.querySelector(".custom-select-option.selected");
      if (selected) {
        prompt_caching = selected.dataset.value === "null" ? null : (selected.dataset.value === "true" ? true : false);
      } else {
        prompt_caching = _models[index]?.prompt_caching ?? null;
      }
    }

    const thinkingWrapper = card.querySelector(".thinking-dropdown")?.closest(".custom-select-wrapper");
    let thinking_enabled = null;
    if (thinkingWrapper) {
      const selected = thinkingWrapper.querySelector(".custom-select-option.selected");
      if (selected) {
        thinking_enabled = selected.dataset.value === "null" ? null : (selected.dataset.value === "true");
      } else {
        thinking_enabled = _models[index]?.thinking_enabled ?? null;
      }
    }

    const effortWrapper = card.querySelector(".reasoning-effort-dropdown")?.closest(".custom-select-wrapper");
    let reasoning_effort = null;
    if (effortWrapper) {
      const selected = effortWrapper.querySelector(".custom-select-option.selected");
      if (selected) {
        reasoning_effort = selected.dataset.value || null;
      } else {
        reasoning_effort = _models[index]?.reasoning_effort || null;
      }
    }
    ...
    return {
      ...
      compression_overrides: _readCompressionOverrides(card, index)
    };
  }

  // Read per-model compression overrides from the card's collapsible section.
  // Returns null if no overrides are set (all fields empty), otherwise a hash.
  function _readCompressionOverrides(card, index) {
    const overrides = {};
    card.querySelectorAll("[data-override]").forEach(input => {
      ...
      if (val !== "") {
        ...
        if (!isNaN(num) && num > 0) overrides[key] = num;
      }
    });
  ...
  async function _saveModel(index) {
    ...
    const payload = {
      ...
      prompt_caching:   updated.prompt_caching,
    ...
    // Include per-model compression overrides (null clears, object sets)
    if (updated.compression_overrides !== undefined) {
      payload.compression_overrides = updated.compression_overrides;
    }
  ...
  async function _loadCompressionSettings() {
    try {
      const res = await fetch("/api/settings");
      const data = await res.json();
      const s = data.settings || {};

      const enableEl = document.getElementById("compression-enable-input");
      const tokenEl  = document.getElementById("compression-token-threshold");
      const msgEl    = document.getElementById("compression-message-threshold");
      const recentEl = document.getElementById("compression-max-recent");
      const targetEl = document.getElementById("compression-target-tokens");
      const idleTokEl = document.getElementById("compression-idle-threshold");
      const idleDelayEl = document.getElementById("compression-idle-delay");
      const rawLoggingEl = document.getElementById("raw-response-logging-input");

      if (enableEl) enableEl.checked = s.enable_compression !== false;
      if (rawLoggingEl) rawLoggingEl.checked = s.raw_response_logging_enabled === true;
      if (tokenEl)  tokenEl.value  = s.compression_token_threshold   || 150000;
      if (msgEl)    msgEl.value    = s.compression_message_threshold || 200;
      if (recentEl) recentEl.value = s.compression_max_recent_messages || 20;
      if (targetEl) targetEl.value = s.compression_target_tokens     || 10000;
      if (idleTokEl) idleTokEl.value = s.idle_compression_threshold  || 20000;
      if (idleDelayEl) idleDelayEl.value = s.idle_compression_delay  || 180;

      _bindCompressionSave();
    } catch (e) {
      console.error("Failed to load compression settings:", e);
    }
  }

  function _bindCompressionSave() {
    const btn = document.getElementById("btn-save-compression-settings");
    if (!btn) return;
    ...
    newBtn.addEventListener("click", async () => {
      ...
      const settings = {
        enable_compression:             document.getElementById("compression-enable-input")?.checked,
        raw_response_logging_enabled:   document.getElementById("raw-response-logging-input")?.checked,
        compression_token_threshold:    parseInt(document.getElementById("compression-token-threshold")?.value) || 150000,
        compression_message_threshold:  parseInt(document.getElementById("compression-message-threshold")?.value) || 200,
        compression_max_recent_messages: parseInt(document.getElementById("compression-max-recent")?.value) || 20,
        compression_target_tokens:      parseInt(document.getElementById("compression-target-tokens")?.value) || 10000,
        idle_compression_threshold:     parseInt(document.getElementById("compression-idle-threshold")?.value) || 20000,
        idle_compression_delay:         parseInt(document.getElementById("compression-idle-delay")?.value) || 180
      };
      ...
      try {
        ...
        if (res.ok) {
          if (statusEl) {
            statusEl.textContent = I18n.t("settings.compression.saved");
          ...
          if (statusEl) {
            statusEl.textContent = I18n.t("settings.compression.saveFailed") + ": " + (err.error || "");
            statusEl.style.color = "var(--color-error)";
          }
        }
      } catch (e) {
        if (statusEl) {
          statusEl.textContent = I18n.t("settings.compression.saveFailed");
          statusEl.style.color = "var(--color-error)";
        }
      }
    });
