...
RSpec.describe Clacky::Client, "Responses API streaming callbacks" do
  ...
  it "emits reasoning and text deltas while accumulating the final response" do
    ...
    raw_stream = [
      double("reasoning_delta", type: :"response.reasoning.delta", delta: "Think"),
      double("text_delta_1", type: :"response.output_text.delta", delta: "Hello"),
      double("text_delta_2", type: :"response.output_text.delta", delta: " world"),
      double("completed", type: :"response.completed", response: completed_response)
    ]
